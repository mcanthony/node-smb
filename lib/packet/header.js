...
Header.prototype = {
  ...
  toBuffer: function() {
    ...
    buffer.writeUInt32LE( this.protocolId, 0 )
    buffer.writeUInt16LE( this.structureSize, 4 )
    buffer.writeUInt16LE( this.creditCharge, 6 )
    buffer.writeUInt32LE( this.status, 8 )
    buffer.writeUInt16LE( this.channelSequence, 8 )
    buffer.writeUInt16LE( this.command, 12 )
    buffer.writeUInt16LE( this.credit, 14 )
    buffer.writeUInt32LE( this.flags, 16 )
    buffer.writeUInt32LE( this.nextCommand, 20 )
    
    buffer.write( this.messageId, 24, 8, 'hex' )
    
    if( this.flags & SMB.PACKET.ASYNC_COMMAND ) {
      buffer.write( this.asyncId, 32, 8, 'hex' )
    } else {
      // this.reserved = 0x00000000
      buffer.writeUInt32LE( this.treeId, 36 )
    }
    
    buffer.write( this.sessionId, 40, 8, 'hex' )
    buffer.write( this.signature, 48, 16, 'hex' )
